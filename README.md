...
- ### request.py :
  This is a sript which can be used for testing. It is used to send request to the Web service (run via predict.py) and display the output (prediction). To make it convinient to test, three sample patient data has been added in the script.
...
From one terminal session run the following command to host the prediction model as a web service: <br>

_gunicorn --bind 0.0.0.0:9696 predict.py_

From other terminal session from the cloned project directory, execute the following command to make a request to this web service: <br>

_python request.py_

## 6. Steps to train the model 

To train the model follow the below steps.

1. Activate the virtual environment if not done already. Follow the steps in section 3 above. <br>

2. Clone the repo using below command: <br>
